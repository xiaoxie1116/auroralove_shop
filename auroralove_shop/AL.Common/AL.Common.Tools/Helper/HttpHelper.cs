...
namespace AL.Common.Tools.Helper
{
    ...
    public class HttpHelper
    {
        ...
        /// /异步页面请求
        /// </summary>
        /// <param name="url">请求接口地址 EG："https://localhost:44399/Product/SyncGetRotaionImages"</param>
        /// <param name="postDataStr"></param>
        /// <param name="encoding"></param>
        /// <param name="timeout"></param>
        /// <returns></returns>
        public static async Task<string> CallWebPageAsync(string url, string postDataStr, Encoding encoding, int timeout)
        {
            string rStr = "";
            try
            {
    ...
    public class HttpHelper<T> where T : new()
    {
        public static async Task<T> ClientPost(string action, dynamic parameter)
        {
            HttpClient client = new HttpClient();
            dynamic str = JsonConvert.SerializeObject(parameter);
            StringContent stringContent = new StringContent(str, Encoding.UTF8, "application/json");
            var a = await (await client.PostAsync(action, stringContent)).Content.ReadAsStringAsync();
            T result = JsonConvert.DeserializeObject<T>(a);
            return result;
        }

        public static async Task<T> ClientGet(string action)
        {
            HttpClient client = new HttpClient();
            var a = await (await client.GetAsync(action)).Content.ReadAsStringAsync();
            T result = JsonConvert.DeserializeObject<T>(a);
            return result;
        }
    }
